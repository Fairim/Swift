...
struct DailySectionView: View{
    ...
    var body: some View{
        ZStack(alignment: .top) {
            ...
            VStack(spacing: 6) {
                ...
                ForEach(Array(displayedDailyWeather.enumerated()), id: \.offset) { _, dayWeather in
                    dailyRowView(for: dayWeather)
                }
            }
            .padding(.vertical, 8)
        }
    ...
    private func dailyRowView(for dayWeather: DailyWeather) -> some View {
        ...
        return ZStack {
            ...
            HStack(spacing: 2) {
                ...
                RoundedRectangle(cornerRadius: 10)
                    ...
                    .frame(width: screenWidth / 4, height: screenHeight / 80, alignment: .trailing)
                    .padding(.horizontal, 10)
                
                Text("\(nightTemperature)°")
                    .font(.system(size: screenWidth / 20, weight: .bold, design: .default))
                    .frame(maxWidth: screenWidth / 11, alignment: .trailing)
            }
        }
    }

    private var displayedDailyWeather: [DailyWeather] {
        Array(masDailyWeather.prefix(7))
    }
